var PrisnaYWTCommon = {
	...
	getHeadingContainer: function(_item) {
		...
		var section = jQuery(_item).parents(".prisna_ywt_section");
		var heading = section.prevUntil(".prisna_ywt_heading");
...
PrisnaYWTCommon.Dependencies = {
	...
	_record: function(_target) {
		...
		var foo = _target[0];
		var section = jQuery(foo).parents(".prisna_ywt_section");
	...
	simulate: function(_section) {
		...
		if (PrisnaYWTCommon.CSS.hasClass(_section, "prisna_ywt_toggle"))
			type = "toggle";
...
PrisnaYWTCommon.Heading = function(_target) {
...
PrisnaYWTCommon.Heading.prototype._initialize_events = function() {
	
	PrisnaYWTCommon.addEvent(this._target, "click", function(e) {

		var owner = PrisnaYWTCommon.getOwnerParent(PrisnaYWTCommon.getOwner(e));
		owner._owner._click();
...
PrisnaYWTCommon.Heading.prototype.isShowing = function() {
	
	return !PrisnaYWTCommon.CSS.hasClass(this._target, "prisna_ywt_heading_hiding");
	
};

PrisnaYWTCommon.Heading.prototype._click = function() {
	
	this._show(!this.isShowing());
	
};

PrisnaYWTCommon.Heading.prototype._show = function(_state, _now) {

	PrisnaYWTCommon.CSS.chooseClass(this._target, _state, "prisna_ywt_heading_showing", "prisna_ywt_heading_hiding");

	var items = jQuery(this._items);
	items = items.not(".prisna_ywt_no_display.prisna_ywt_section_tabbed_2");
	...
	if (_now === true) {
		...
		PrisnaYWTCommon.Cookie.set("prisna_ywt_heading_" + this._id, _state ? "true" : "false", 10, false, false, false);
...
PrisnaYWTCommon.Heading.prototype._click_items = function(_items, _delayed) {

	if (_delayed)
		setTimeout(function() {
			for (var i=0; i<_items.length; i++)
				PrisnaYWTCommon.Dependencies.simulate(_items[i]);
		}, 200);
	else
		for (var i=0; i<_items.length; i++)
			PrisnaYWTCommon.Dependencies.simulate(_items[i]);
...
PrisnaYWTCommon.Heading.prototype._click_item = function(_item, _name) {
	
	var type;
	
	if (PrisnaYWTCommon.CSS.hasClass(_item, "prisna_ywt_toggle"))
		type = "toggle";
...
PrisnaYWTCommon.Heading.prototype._initialize_items = function() {

	this._target._owner = this;
	
	var item = this._get_next_sibling(this._target);

	while (item != null && item.className.match(/\bprisna_ywt_section_tabbed_\d+\b/)) {
		this._items.push(item);
		item = this._get_next_sibling(item);
	}
	
	var cookie = PrisnaYWTCommon.Cookie.get("prisna_ywt_heading_" + this._id);
	if (cookie == "true" || cookie == "false")
		this._show(cookie == "true", true);
	
};
	
PrisnaYWTCommon.Heading.prototype._get_next_sibling = function(_reference) {
...
PrisnaYWTCommon.Heading.prototype._set_styles = function() {
		
	PrisnaYWTCommon.CSS.addClass(this._target, "prisna_ywt_heading_enabled");
	
};

PrisnaYWTCommon.Cookie = {
	...
	remove: function(name, path, domain) {
		if (PrisnaYWTCommon.Cookie.get(name)) document.cookie = name + '=' +
				((path) ? ';path=' + path : '') +
				((domain) ? ';domain=' + domain : '') +
...
PrisnaYWTCommon.initializeTooltip = function(_target) {
...
PrisnaYWTCommon.CSS = {
	...
	hasClass: function(_element, className) { 
		_element = PrisnaYWTCommon.$(_element);
		if (!_element)
			return;
		if(_element&&className&&_element.className) { 
			return new RegExp('\\b'+PrisnaYWTCommon.trim(className)+'\\b').test(_element.className);
		}
		return false;
	},
	...
	addClass: function(_element, className) {
		...
		if(_element&&className) {
			if(!PrisnaYWTCommon.CSS.hasClass(_element, className)) {
				className = PrisnaYWTCommon.trim(className);
				if(_element.className) { 
	...
	conditionClass: function(_element, className, shouldShow) { 
		if(shouldShow) { 
			PrisnaYWTCommon.CSS.addClass(_element, className);
		}
		else { 
			PrisnaYWTCommon.CSS.removeClass(_element, className);
		}
	},
		
	chooseClass: function(_element, expression, trueClass, falseClass) { 
		PrisnaYWTCommon.CSS.conditionClass(_element, trueClass, expression);
		PrisnaYWTCommon.CSS.conditionClass(_element, falseClass, !expression);
	},
	...
	toggleClass: function(_element, className) {
		_element = PrisnaYWTCommon.$(_element);
		if(PrisnaYWTCommon.CSS.hasClass(_element, className)) {
			return PrisnaYWTCommon.CSS.removeClass(_element, className);
		}
		else {
			return PrisnaYWTCommon.CSS.addClass(_element, className);
		}
	},
...
PrisnaYWTCommon.Tabs = function(level) {
...
PrisnaYWTCommon.Tabs.prototype.registerTab = function(param, callback) {
	
	var target = PrisnaYWTCommon.$(param + "_menu");
	...
	PrisnaYWTCommon.addEvent(PrisnaYWTCommon.$(param + "_menu"), "click", function(e) {
...
PrisnaYWTCommon.Tabs.prototype.selectTab = function(name) {
...
PrisnaYWTCommon.Tabs.prototype.setField = function(name) {
	
	var aux = this.level !== 0 ? "_" + this.level : "";
	PrisnaYWTCommon.$("prisna_tab" + aux).value = name;
	
};

PrisnaYWTCommon.Tabs.prototype.displayTab = function(name, state) {

	PrisnaYWTCommon.CSS.chooseClass(PrisnaYWTCommon.$(name + "_tab"), state, "prisna_ywt_display", "prisna_ywt_no_display");

};

PrisnaYWTCommon.Tabs.prototype.getSelected = function() {
	
	for(var i=0; i<this.tabs.length; i++) 
		if(PrisnaYWTCommon.CSS.hasClass(this.tabs[i] + "_menu", "prisna_wp_translate_ui_tab_selected")) 
			return this.tabs[i];
	
	return "";
};

PrisnaYWTCommon.Tabs.prototype.selectMenu = function(name, state) {

	PrisnaYWTCommon.CSS.chooseClass(PrisnaYWTCommon.$(name + "_menu"), state, "prisna_ywt_ui_tab_selected", "prisna_ywt_ui_tab_unselected");
