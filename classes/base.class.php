...
abstract class PrisnaYWTItem {
	...
	public function render($_options, $_html_encode=false) {
		...
		$result = PrisnaYWTCommon::renderObject($this, $_options, $_html_encode);
	...
	protected function _json() {
		...
		foreach ($this as $property => $value)
			if (!PrisnaYWTCommon::endsWith($property, '_json'))
				$fields[] = $property;

		foreach ($fields as $value)
			if (substr($value, 0, 1) != '_')
				$this->setProperty($value . '_json', PrisnaYWTCommon::jsonCompatible($this->getProperty($value)));

	}

}

abstract class PrisnaYWTField {
	...
	public function satisfyDependence($_fields) {
		...
		if (PrisnaYWTValidator::isEmpty($this->dependence))
			return;

		$this->_dependence = PrisnaYWTCommon::getArrayItems($this->dependence, $_fields);
	...
	protected function _has_dependence() {
		...
		return !is_null($this->dependence) && !PrisnaYWTValidator::isEmpty($this->dependence);
	...
	protected function _dependence_show() {
		...
		foreach ($this->_dependence as $field)
			if (PrisnaYWTCommon::inArray($field->value, $this->dependence_show_value))
				$result[] = $field->id;
	...
	protected function _render($_options, $_html_encode) {
		...
		$result = PrisnaYWTCommon::renderObject($this, $options, $_html_encode);
...
class PrisnaYWTColorField extends PrisnaYWTField {
...
class PrisnaYWTTextField extends PrisnaYWTField {
...
class PrisnaYWTKeyField extends PrisnaYWTField {

	public $empty_validate_message;
	
	protected function _empty_validate() {
	...
	public function output($_html_encode=false) {
		...
		$result = parent::render(array(
			'type' => 'file',
			'content' => '/admin/key.tpl',
			'meta_tag_rules' => array(
				array(
					'expression' => $this->_empty_validate(),
					'tag' => 'empty_validate'
				)
			)
		), $_html_encode);
...
class PrisnaYWTPremiumField extends PrisnaYWTField {

	public $images_path;

	public function output($_html_encode=false) {

		$this->images_path = PRISNA_YWT__IMAGES;
...
class PrisnaYWTUsageField extends PrisnaYWTField {
...
class PrisnaYWTHeadingField extends PrisnaYWTField {
	...
	public function output($_html_encode=false) {
		...
		$result = parent::render(array(
			...
			'meta_tag_rules' => array(
				...
				array(
					'expression' => !PrisnaYWTValidator::isEmpty($this->description_message),
					'tag' => 'description'
				)
			)
		), $_html_encode);
...
class PrisnaYWTHeading2Field extends PrisnaYWTField {
...
class PrisnaYWTSitemapviewField extends PrisnaYWTField {
	...
	public function __construct($_properties) {
		...
		$this->xml_url = PrisnaYWTCommon::getHomeUrl('/' . PrisnaYWTConfig::getSitemapFilename() . '.xml');
		$this->csv_url = PrisnaYWTCommon::getHomeUrl('/' . PrisnaYWTConfig::getSitemapFilename() . '.csv');
...
class PrisnaYWTRuleField extends PrisnaYWTField {
	...
	public function __construct($_properties) {
		...
		$this->priority = new PrisnaYWTSelectField(array(
			'title_message' => '',
			'description_message' => '',
		...
		$this->changefreq = new PrisnaYWTSelectField(array(
			'title_message' => '',
			'description_message' => '',
			'id' => $this->id . '_changefreq',
			'type' => 'select',
			'values' => array(
				'always' => PrisnaYWTMessage::get('changefreq_always'),
				'hourly' => PrisnaYWTMessage::get('changefreq_hourly'),
				'daily' => PrisnaYWTMessage::get('changefreq_daily'),
				'weekly' => PrisnaYWTMessage::get('changefreq_weekly'),
				'monthly' => PrisnaYWTMessage::get('changefreq_monthly'),
				'yearly' => PrisnaYWTMessage::get('changefreq_yearly'),
				'never' => PrisnaYWTMessage::get('changefreq_never')
			),
...
class PrisnaYWTToggleField extends PrisnaYWTField {
...
class PrisnaYWTRangeField extends PrisnaYWTField {
...
class PrisnaYWTTextareaField extends PrisnaYWTField {
...
class PrisnaYWTExportField extends PrisnaYWTField {
...
class PrisnaYWTLanguageOptionField extends PrisnaYWTField {
	...
	public function __construct($_properties) {
		...
		$this->base_url = PRISNA_YWT__IMAGES . '/';
...
class PrisnaYWTLanguageOrderOptionField extends PrisnaYWTField {
...
class PrisnaYWTLanguageField extends PrisnaYWTField {
	...
	protected function _set_options() {
		...
		for ($i=0; $i<$this->columns; $i++)
			$this->_collections[$i] = new PrisnaYWTItemCollection();
		...
		foreach ($this->values as $key => $value) {
			...
			$this->_collections[$group]->add(new PrisnaYWTLanguageOptionField((object) array(
				'id' => PrisnaYWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
	...
	protected function _set_order_options() {
		...
		$this->_collection_order = new PrisnaYWTItemCollection();
		...
		foreach ($this->values as $key => $value) {
			...
			$items[array_search($key, $values)] = new PrisnaYWTLanguageOrderOptionField((object) array(
				'id' => PrisnaYWTCommon::cleanId($this->id . '_order_' . $key, '_'),
				'option' => $key,
				'value' => $value
			));
...
class PrisnaYWTCheckboxOptionField extends PrisnaYWTField {
...
class PrisnaYWTCheckboxField extends PrisnaYWTField {
	...
	protected function _set_options() {
		...
		$this->collection = new PrisnaYWTItemCollection();

		foreach ($this->values as $key => $value) {

			$this->collection->add(new PrisnaYWTCheckboxOptionField((object) array(
				'id' => PrisnaYWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
				'checked' => is_array($this->value) ? in_array((string) $key, $this->value, true) : false,
				'option' => $key,
...
class PrisnaYWTExcategoryField extends PrisnaYWTExclitemField {
...
class PrisnaYWTExpageField extends PrisnaYWTExclitemField {
...
class PrisnaYWTExpostField extends PrisnaYWTExclitemField {
...
class PrisnaYWTExclitemOptionField extends PrisnaYWTField {
...
class PrisnaYWTExclitemField extends PrisnaYWTCheckboxField {
	...
	protected function _sort() {
		...
		$result = array();
		$temp = PrisnaYWTCommon::chain('id', 'parent', 'value', $this->values);
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaYWTExclitemOptionField((object) array(
				'id' => PrisnaYWTCommon::cleanId($this->id . '_' . $key, '_'),
				'name' => $this->id,
				'checked' => is_array($this->value) ? in_array((string) $key, $this->value, true) : false,
				'option' => $key,
...
class PrisnaYWTRadioOptionField extends PrisnaYWTField {
...
class PrisnaYWTRadioField extends PrisnaYWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaYWTRadioOptionField((object) array(
				'id' => $this->id . '_' . $key,
				'name' => $this->id,
...
class PrisnaYWTVisualOptionField extends PrisnaYWTField {
...
class PrisnaYWTVisualField extends PrisnaYWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaYWTVisualOptionField((object) array(
				'id' => $this->id . '_' . $key,
				'name' => $this->id,
				'checked' => $this->value == $key,
				'option' => $key,
...
class PrisnaYWTSelectOptionField extends PrisnaYWTField {
...
class PrisnaYWTSelectField extends PrisnaYWTField {
	...
	protected function _set_options() {
		...
		foreach ($this->values as $key => $value) {
			...
			$this->collection->add(new PrisnaYWTSelectOptionField((object) array(
	...
	public function output($_template='select.tpl', $_html_encode=false) {
		...
		$result = parent::render(array(
			...
			'meta_tag_rules' => array(
				array(
					'expression' => property_exists($this, 'post_id') && !PrisnaYWTValidator::isEmpty($this->post_id),
					'tag' => 'has_post_id'
				)
			)
		), $_html_encode);
...
class PrisnaYWTTranslationsField extends PrisnaYWTField {
	...
	protected function _initialize_resource_field($_properties) {
		...
		$this->_resource_field = new PrisnaYWTSelectField($_resource_field);
	...
	public static function getFilesPaths($_folder, $_globals=false) {
		...
		foreach ($contents as $name) {
			$file = $_folder . '/' . $name;
			if (PrisnaYWTCommon::endsWith($name, '.xml') && !PrisnaYWTCommon::startsWith($name, 'permalinks') && !PrisnaYWTCommon::startsWith($name, 'log_') && @is_file($file) && ($_globals || !PrisnaYWTCommon::endsWith($name, 'global.xml')))
				$result[] = $file;
		}
	...
	protected function _get_files_scopes() {
		...
		foreach ($files as $file) {
			...
			$contents = PrisnaYWTFileHandler::read($file);
...
class PrisnaYWTSeologField extends PrisnaYWTField {
	...
	public function __construct($_properties) {
		...
		parent::__construct($_properties);
		$this->_file = PrisnaYWTCommon::getLogFile(PrisnaYWTCommon::getTestApiKey());
		$this->empty_message = PrisnaYWTMessage::get('seo_log_empty_message');
	...
	protected function _render_log($_template='log.tpl', $_html_encode=false) {
		...
		for ($i=$count-1; $i>=0; $i--) {
			...
			$result .= PrisnaYWTCommon::renderObject((object) $entry, array(
				'type' => 'file',
				'content' => '/admin/' . $_template,
				'meta_tag_rules' => array(
					array(
	...
	protected function _get_log() {
		...
		$contents = PrisnaYWTFileHandler::read($this->_file);
...
class PrisnaYWTItemCollection {
	...
	public function render($_options, $_html_encode=false) {
		...
		$result = PrisnaYWTCommon::renderObject($object, $_options, $_html_encode);
